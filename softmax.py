...
class SoftmaxRegression:
    ...
    def cost(self, X, Y, W=None):
        ...
        exp_WX = np.exp(W_X)
        probabilities = exp_WX / np.array([np.sum(n) for n in exp_WX.T])

        indicator_prob = (np.log(probabilities) * indicator)

        cost = - np.sum(indicator_prob) / N

        gradient = -np.matmul((indicator - probabilities), X.T) / N
    ...
    def predict(self, X):
        ...
        exp_WX = np.exp(W_X)
        ex_tot = np.array([np.sum(n) for n in exp_WX.T])
        inv_ex_tot = 1 / ex_tot
        probabilities = exp_WX * inv_ex_tot

        predicted_classes = np.apply_along_axis(np.argmax, 0, probabilities)
